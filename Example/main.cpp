...
int main( int argc, char * argv[] )
{
	...
	FtpServer.SetDataPortRange( 30100, 900 ); // data TCP-Port range = [30100-30999]
	...
	#ifdef WIN32
		CFtpServer::CUserEntry *pUser = FtpServer.AddUser( "test", "pass", "C:\\" );
	#else
		CFtpServer::CUserEntry *pUser = FtpServer.AddUser( "test", "pass", "/" );
	#endif

	CFtpServer::CUserEntry *pAnonymousUser = FtpServer.AddUser( "anonymous", NULL, "C:\\dir" );
	if( pAnonymousUser ) {
	...
	if( pUser ) {
		...
		if( FtpServer.StartListening( INADDR_ANY, 30021 ) ) {
